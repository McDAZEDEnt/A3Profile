...
isSOG=isClass(configFile>>"cfgPatches">>"data_f_vietnam");
r_RTOBP=["B_RadioBag_01_base_F"]+r_RTOBP;
r_RTOBP=(if(isSOG)then{["vn_o_pack_t884_01","vn_b_pack_03_02","vn_b_pack_trp_04_02","vn_b_pack_prc77_01","vn_b_pack_lw_06"]}else{[]})+r_RTOBP;
if(isClass(configFile>>"cfgPatches">>"CUP_Creatures_People_Core"))then{r_RTOBP=["CUP_B_Kombat_Radio_Olive","CUP_B_Motherlode_Radio_MTP","CUP_B_Predator_Radio_MTP"]+r_RTOBP};
missionNamespace setVariable["r_p",player];
r_radios=["ItemRadio"];
if(isSOG)then{player linkItem(switch(playerSide)do{case EAST:{"vn_o_item_radio_m252"};default{"vn_b_item_radio_urc10"}});r_radios=["vn_o_item_radio_m252","vn_b_item_radio_urc10"];
if(playerSide==EAST)then{r_radios=r_radios-["vn_b_item_radio_urc10"]}else{r_radios=r_radios-["vn_o_item_radio_m252"]};
