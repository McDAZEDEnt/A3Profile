...
r_p addEventHandler["Put",{if(_this#2 in r_radios||(r_RTOBP findIf{unitBackpack r_p isKindOf _x}>=0))then{if(r_p call r_RC)exitWith{};call r_allOff;{deleteVehicle(_x#1)}forEach r_f;r_f=[];{r_p remoteExecCall["r_out",_x]}forEach(allPlayers-[r_p]select{side _x==playerSide});
