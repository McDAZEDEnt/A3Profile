...
if((!alive r_p)||(!isAbleToBreathe r_p)||(isObjectHidden r_p))exitWith{if(!isAbleToBreathe r_p)then{[r_p,0]remoteExecCall["setPlayerVONVolume"];5 enableChannel[false,false];{_x enableChannel[false,false]}count r_CH+[0];
...
if(alive r_p)then{(ch9+5)enableChannel[false,false]};
{(_x#0)displayRemoveEventHandler["KeyUp",_x#1]}forEach[[findDisplay 46,kpUp],[findDisplay 312,kpUpZ]];r_dn_=true;r_up_=false;
private _eh=["KeyUp","if(!r_up_ && _this#1 in(actionKeys'pushToTalk'+actionKeys'pushToTalkDirect'+actionKeys'pushToTalkGroup'+actionKeys'pushToTalkVehicle'+actionKeys'pushToTalkSide'+actionKeys'pushToTalkCommand'))then{call r_dn}"];
