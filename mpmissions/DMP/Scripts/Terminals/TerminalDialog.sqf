...
if(dmpTerminalMode=="SECURITY")exitWith{
	_remotes=_object getVariable"dmpTerminalSubsystems";
	if(dmpDebug)then{hint format["Remotes:\n\n%1",_remotes]};
	{if!(alive _x)then{_remotes=_remotes-[_x]}}forEach _remotes;
	{
	if!(_x in _remotes)then{
		if!(alive _x)exitWith{};
		if((_x distance _object)<_radius)then{_remotes pushBack _x};
	};
	}forEach allUnitsUAV;
