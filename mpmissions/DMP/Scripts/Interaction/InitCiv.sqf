...
if!(_man in dmpInteractRegistered)then{
	//"(((_this distance _target)<5)AND(cursorObject==_target))",
...
if((random 100)<dmpCivKnowledgeLeader)then{
	_targets=_pos nearEntities["Man",dmpCivKnowledgeRadius];
...
if((random 100)<dmpCivKnowledgeHVT)then{
	_targets=_pos nearEntities["Man",dmpCivKnowledgeRadius];
...
if((random 100)<dmpCivKnowledgeTroop)then{
	_targets=_pos nearEntities["Man",dmpCivKnowledgeRadius];
	_targets=_targets+(_pos nearEntities["LandVehicle",dmpCivKnowledgeRadius]);
...
if((random 100)<dmpCivKnowledgeCaptive)then{
	_targets=_pos nearEntities["Man",dmpCivKnowledgeRadius];
...
if((random 100)<dmpCivKnowledgeCache)then{
	_targets=_pos nearEntities["ReammoBox_F",dmpCivKnowledgeRadius];
...
if((random 100)<dmpCivKnowledgeSpecial)then{
	_targets=_pos nearEntities["DMP_KnowledgeSpecial",dmpCivKnowledgeRadius];
	if((count _targets)==0)exitWith{};
