// Gets a response from a soldier's variables and returns it
private["_man","_action","_text","_out","_manKnowledge","_c","_willTalk","_leaders","_HVT","_groups","_captives","_caches","_special","_index","_languagePlayer","_languageSoldier","_side","_vehicle","_displayName"];
_man=_this select 0;
...
if((count _manKnowledge)!=7)then{_man call DMP_fnc_InitSoldier};
_c=0;
_willTalk=_manKnowledge select _c;_c=_c+1;
...
if!(_willTalk)then{_action="";_out=selectRandom["I don't know anything.","Sorry, I don't know.","No idea.","I haven't seen anything.","I know nothing about that."]};

_index=3;
if((side player)==WEST)then{_index=0};
if((side player)==EAST)then{_index=1};
if((side player)==RESISTANCE)then{_index=2};
_languagePlayer=toUpper(dmpLanguages select _index);
_side=_man call DMP_fnc_GetSide;
_index=3;
if(_side==WEST)then{_index=0};
if(_side==EAST)then{_index=1};
if(_side==RESISTANCE)then{_index=2};
_languageSoldier=toUpper(dmpLanguages select _index);
_vehicle=objNull;
_displayName="";

if!(_languagePlayer==_languageSoldier)then{
...
if(_action=="TROOPS")then{
	...
	_vehicle=vehicle(leader(_groups select 0));
	if!(_vehicle isKindOf"Man")then{
		_displayName=(typeOf _vehicle)call DMP_fnc_GetDisplayName;
		_out=format["There is a %1 here.",_displayName];
	};
