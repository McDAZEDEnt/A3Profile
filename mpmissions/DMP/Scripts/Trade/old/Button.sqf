...
if(_code=="Buy")exitWith{
	...
	_itemType=_item call dmp_fnc_ItemType;
	...
	if(_itemType=="AttachmentM")then{if(player canAdd[_item,1])then{player addMagazine _item;_canAdd=true}else{hintSilent"Not enough space"}};
	if(_itemType=="AttachmentO")then{if(player canAdd[_item,1])then{player addMagazine _item;_canAdd=true}else{hintSilent"Not enough space"}};
	if(_itemType=="AttachmentP")then{if(player canAdd[_item,1])then{player addMagazine _item;_canAdd=true}else{hintSilent"Not enough space"}};
	if(_itemType=="AttachmentB")then{if(player canAdd[_item,1])then{player addMagazine _item;_canAdd=true}else{hintSilent"Not enough space"}};
	if(_canAdd)then{dmpPlayerCash=dmpPlayerCash-_price;systemChat format["Bought %1",_item call dmp_fnc_GetDisplayName]};
	sleep .3;
