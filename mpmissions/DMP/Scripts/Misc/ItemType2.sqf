...
if(getNumber(configFile>>"cfgWeapons">>_item>>"type")==131072)then{_type="Item"};
...
if(getNumber(configFile>>"cfgWeapons">>_item>>"itemInfo">>"type")==101)then{_type="AttachmentM"}; // Muzzle
if(getNumber(configFile>>"cfgWeapons">>_item>>"itemInfo">>"type")==201)then{_type="AttachmentO"}; // Optic
if(getNumber(configFile>>"cfgWeapons">>_item>>"itemInfo">>"type")==301)then{_type="AttachmentP"}; // Pointer
if(getNumber(configFile>>"cfgWeapons">>_item>>"itemInfo">>"type")==302)then{_type="AttachmentB"}; // Bipod
