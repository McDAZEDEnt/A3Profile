...
if (isServer) then {
	...
	if (isClass (configFile >> "CfgPatches" >> "ace_medical") || isClass (configFile >> "CfgPatches" >> "ace_main")) then {
		ais_ace_shutDown = true;
		["AIS: AIS shutdown itself because ACE medical system is also running. ACE medical and AIS cant work at the same time."] call BIS_fnc_logFormat;
	};
