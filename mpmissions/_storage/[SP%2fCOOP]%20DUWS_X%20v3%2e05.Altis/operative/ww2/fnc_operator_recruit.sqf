...
_soldier addEventHandler ["Reloaded", {
	params ["_soldier", "_weapon", "_muzzle", "_newmag", ["_oldmag", ["","","",""]]];
	if (_oldmag select 2 isEqualType "") then {_soldier addMagazine (_newmag select 0)} else {_soldier addMagazine (_oldmag select 0)};
}];
...
//_soldier addHeadgear _selected_soldier_headgear;  LT. Dang Edit Make specOps have default headgear
