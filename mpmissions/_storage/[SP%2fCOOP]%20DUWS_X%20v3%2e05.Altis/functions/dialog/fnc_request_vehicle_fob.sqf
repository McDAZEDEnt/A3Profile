...
_spawnPos = [[(_spawnPos select 0), (_spawnPos select 1)],30,50,15,0,0.3,0,[],[]] call BIS_fnc_findSafePos;
...
if (_setmrkr) then {
	_markername = format["lbl%1%2",_spawnPos]; // Define marker name
	//hint _markername;
	...
	str(_markername) setMarkerSize [.5,.5]; 
	str(_markername) setMarkerText localize "STR_DUWS_X_uhaz94ur";
	sleep 60;
